...
exports.handler = async (event, context) => {
  console.log('Add-model function called:', event.httpMethod, event.path);
  ...
  try {
    ...
    // Make initial request to FAL nano-banana edit endpoint (used for model/lifestyle generation)
    ...
    while (attempts < maxAttempts) {
      ...
      if (statusData.status === 'COMPLETED') {
        ...
        throw new Error('Model/lifestyle generation failed');
      }
    ...
    if (attempts >= maxAttempts) {
      throw new Error('Model/lifestyle generation timeout - operation took too long');
    }
    ...
    if (!resultResponse.ok) {
      throw new Error('Failed to retrieve generated lifestyle image');
    }
          ...
          await supabase.from('usage_tracking').insert([
            {
              ...
              count: 1,
              metadata: {
                service: 'fal-nano-banana-add-model',
                prompt,
